...
RUN apk add --no-cache curl bash && mkdir /build && cd /build && \
 ...
 curl -L 'http://npc.nos-eastchina1.126.net/dl/jq_1.5_linux_amd64.tar.gz' | tar -zx -C /usr/bin && \
 curl -L 'https://npc.nos-eastchina1.126.net/dl/jwks2pem.tar.gz' | tar -zx -C /usr/bin && \
 rm -rf /build
